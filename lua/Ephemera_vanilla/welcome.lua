...
function M.draw()
	...
	local block_keys = {
		...
		-- Search (Optional - keeps the buffer clean)
		'/', '?', 'n', 'N', '*', '#',

		-- Enter (blocked to prevent moving to next line unless on a button)
		'<CR>', '<kEnter>'
	}

	for _, key in ipairs(block_keys) do
		-- Map Normal, Visual, and Operator-pending modes to Nothing
	...
	-- D. Separator & Rose (Aligned to Header Top)
	if show_rose then
		local rose_start_y = header_y
		local total_h = math.max(btn_end_y, rose_start_y + #M.config.flower_art)
	...
	-- IMPORTANT: Sort queue Right-to-Left (High Column to Low Column)
	-- This prevents multi-byte char expansion on the left from shifting indices on the right.
	...
	for _, item in ipairs(draw_queue) do
		...
		-- Use pcall to ignore out-of-bounds errors on resize
